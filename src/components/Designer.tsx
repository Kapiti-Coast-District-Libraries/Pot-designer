...
const ControlGroup = ({ title, icon: Icon, children }: { title: string; icon: any; children: React.ReactNode }) => (
  <div className="mb-6">
    <div className="flex items-center gap-2 mb-3 px-1">
      ...
      <h3 className="text-[10px] font-mono uppercase tracking-[0.15em] text-slate-500">{title}</h3>
    </div>
...
const Slider = ({ label, value, min, max, step = 1, unit = 'mm', onChange }: { label: string; value: number; min: number; max: number; step?: number; unit?: string; onChange: (v: number) => void }) => (
  <div className="space-y-2">
    <div className="flex justify-between items-center">
      ...
      <span className="text-[11px] font-mono text-slate-500 font-medium">{value}{unit}</span>
    </div>
    <input
      type="range"
      min={min}
...
const Select = ({ label, value, options, onChange }: { label: string; value: string; options: { label: string; value: string }[]; onChange: (v: any) => void }) => (
  <div className="space-y-2">
    <label className="text-[11px] font-semibold text-slate-700 block">{label}</label>
    <div className="grid grid-cols-3 gap-2">
      {options.map((opt) => (
        <button
          ...
          className={cn(
            "text-[10px] py-1.5 rounded border transition-all uppercase tracking-wider font-mono font-medium",
            value === opt.value 
              ? "bg-blue-600 text-white border-blue-600 shadow-sm" 
              : "bg-slate-50 text-slate-500 border-slate-200/80 hover:bg-slate-100 hover:text-slate-700 hover:border-slate-300"
          )}
        >
          {opt.label}
        </button>
...
export const Designer: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden">
      {/* Sidebar */}
      <div className="w-[380px] h-full border-r border-slate-200/80 flex flex-col bg-white z-10 shadow-lg">
        <div className="p-6 border-b border-slate-200/80 bg-slate-50/50">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-md font-bold tracking-tight leading-tight text-slate-900">Kapiti Libraries</h1>
                <p className="text-[9px] font-mono text-slate-500 uppercase tracking-widest">Custom Plant Pot Maker</p>
              </div>
            </div>
            <button 
              onClick={randomize}
        ...
        <div className="flex-1 overflow-y-auto p-6 scrollbar-hide">
            ...
            <div className="pt-2">
              <label className="flex items-center justify-between cursor-pointer group">
                <div className="flex items-center gap-2">
                  <AlertTriangle size={14} className={cn("transition-colors", params.enforceOverhangLimit ? "text-amber-500" : "text-slate-400")} />
                  <span className="text-[11px] font-semibold text-slate-700">Enforce 45° Overhang</span>
                </div>
                <div 
                  onClick={() => handleParamChange('enforceOverhangLimit', !params.enforceOverhangLimit)}
                  className={cn(
                    "w-8 h-4 rounded-full transition-colors relative",
                    params.enforceOverhangLimit ? "bg-emerald-500" : "bg-slate-200"
                  )}
                >
                  <div className={cn(
          ...
          <ControlGroup title="Custom Attachments" icon={Plus}>
            <div className="space-y-4">
              <button 
                onClick={addClip}
                className="w-full py-2.5 px-4 bg-blue-600 hover:bg-blue-700 rounded-lg flex items-center justify-center gap-2 text-[11px] font-bold text-white shadow-xs transition-all uppercase tracking-wider"
              >
                <Plus size={14} />
                Add Clip
              </button>
              ...
              <AnimatePresence>
                {params.attachments.map((att) => (
                  <motion.div 
                    ...
                    exit={{ opacity: 0, x: 20 }}
                    className="p-4 bg-slate-50/50 rounded-xl border border-slate-200/80 space-y-4"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-mono text-slate-800 font-semibold truncate max-w-[150px]">{att.name}</span>
                      <button 
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-100">
                        <div className="space-y-1">
                          <span className="text-[9px] text-slate-500 uppercase font-mono font-medium tracking-wider">Mount Width</span>
                          <input 
                            type="number" 
                            value={att.mountingWidth}
                            onChange={(e) => updateAttachment(att.id, { mountingWidth: parseFloat(e.target.value) })}
                            className="w-full bg-slate-50 border border-slate-200/85 hover:border-slate-300 focus:border-blue-500 rounded-lg px-2.5 py-1.5 text-[10px] text-slate-800 font-mono focus:outline-hidden"
                          />
                        </div>
                        <div className="space-y-1">
                          <span className="text-[9px] text-slate-500 uppercase font-mono font-medium tracking-wider">Mount Height</span>
                          <input 
                            type="number" 
            ...
            <div className="flex items-center gap-4">
              <div className="relative group">
                <input 
                  ...
                  className="w-12 h-12 rounded-lg border border-slate-200 bg-transparent cursor-pointer overflow-hidden appearance-none shadow-xs"
                />
              </div>
              <div className="flex-1">
                <p className="text-[11px] font-semibold text-slate-700">Material Finish</p>
                <p className="text-[10px] font-mono text-slate-400 uppercase tracking-wider">{color}</p>
              </div>
            </div>
          </ControlGroup>
        </div>
        ...
        <div className="p-6 border-t border-slate-200 bg-slate-50/50 space-y-3">
          <button
            ...
            className={cn(
              ...
              showAnalysis 
                ...
                : "bg-white text-slate-600 border-slate-200 hover:border-slate-300 hover:bg-slate-50 shadow-xs cursor-pointer"
            )}
          >
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 bg-slate-900 text-white hover:bg-slate-800 border border-slate-900 hover:border-slate-800 py-3.5 rounded-lg text-[12px] font-bold uppercase tracking-widest transition-all disabled:opacity-50 cursor-pointer shadow-sm"
          >
            {isExporting ? (
      ...
      <div className="flex-1 p-6 relative">
        ...
        <div className="absolute top-10 right-10 flex flex-col gap-2 pointer-events-none">
          <div className="bg-white/95 backdrop-blur-md p-5 rounded-2xl border border-slate-200/80 shadow-lg min-w-[210px]">
            <div className="flex items-center gap-2 mb-4">
              <Info size={14} className="text-blue-600" />
              <span className="text-[10px] font-mono uppercase tracking-widest text-slate-500 font-semibold">Print Estimation</span>
            </div>
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                ...
                <span className="text-[13px] font-mono font-bold text-slate-900">{Math.round(stats.weight)}g</span>
              </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-[13px] font-mono font-bold text-emerald-600">${stats.cost.toFixed(2)}</span>
              </div>

              <div className="pt-3 border-t border-slate-100">
                <div className="flex justify-between items-center opacity-70 mb-1">
                  <span className="text-[10px] font-mono uppercase tracking-wider text-slate-400 font-medium">Volume</span>
                  <span className="text-[10px] font-mono text-slate-600 font-semibold">{Math.round(stats.volume)} cm³</span>
                </div>
                <div className="flex justify-between items-center opacity-70">
                  <span className="text-[10px] font-mono uppercase tracking-wider text-slate-400 font-medium">Settings</span>
                  <span className="text-[10px] font-mono text-slate-600 font-semibold">{params.infill}% Infill</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {showAnalysis && (
            <motion.div 
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              className="bg-red-50/95 backdrop-blur-md p-4 rounded-xl border border-red-200 shadow-sm"
            >
              <div className="flex items-center gap-2 mb-1">
                <AlertTriangle size={14} className="text-red-600" />
                <span className="text-[10px] font-mono uppercase tracking-widest text-red-600 font-semibold">Overhang Warning</span>
              </div>
              <p className="text-[10px] text-red-700/80 max-w-[200px] leading-relaxed">Red areas indicate slopes steeper than 45°. These may require supports during 3D printing.</p>
            </motion.div>
          )}
        </div>
      </div>
